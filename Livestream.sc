...
Livestream35 : Livestream {
	...
	*playSound {
		...
		if(~busMade35.isNil){
			...
			~bassBus = Bus.audio(Server.local, 2);
			~synthBus = Bus.audio(Server.local, 2);
			~drumBus = Bus.audio(Server.local, 2);
...
LiveWindow {
	...
	*window {
		...
		if(window.isNil){
			window = Window("LiveWindow", Rect(720, 250, 400, 400)).background_(Color.black);
...
LiveView {
	...
	*view {
		if(view.isNil){
			view = UserView(LiveWindow(), Rect(0, 0, 400, 400));
			^view;
